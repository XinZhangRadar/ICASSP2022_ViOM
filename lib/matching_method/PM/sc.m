    ...
    Xk  = X;
%         c1=mean(Xk);
%     t1=atan2(Xk(:,2)-c1(2), Xk(:,1)-c1(1));
%     c2=mean(Y);
%     t2=atan2(Y(:,2)-c2(2), Y(:,1)-c2(1));
%     [BH1,mean_dist_1]=sc_compute(Xk',t1',mean_dist_global,nbins_theta,nbins_r,r_inner,r_outer,out_vec_1);
%     [BH2,mean_dist_2]=sc_compute(Y',t2',mean_dist_1,nbins_theta,nbins_r,r_inner,r_outer,out_vec_2);
...
%     if size(BH1,1)>=size(BH2,2)
%         [A,B] = canoncorr(BH1(1:size(BH2,1),:),BH2);
%     else
%         [A,B] = canoncorr(  BH1,  BH2(  1:size(BH1,1),:  )   );
%     end
%     BH11 = BH1*A;
%     BH22 = BH2*B;
%     costmat=hist_cost_2(BH11,BH22);
